...
const changeUserStatus = async (user : IRequestUser, payload : IChangeUserStatusPayload ) => {
    ...
    if(isAdminExists.user.role === UserRole.ADMIN && userToChangeStatus.role === UserRole.SUPER_ADMIN){
    ...
    if(isAdminExists.user.role === UserRole.ADMIN && userToChangeStatus.role === UserRole.ADMIN){
...
const changeUserRole = async (user : IRequestUser, payload : IChangeUserRolePayload) => {
    ...
    if(userToChangeRole.role === UserRole.DOCTOR || userToChangeRole.role === UserRole.PATIENT){
